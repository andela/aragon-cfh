...
UserSchema.methods = {
  ...
  addFriend: function (userEmail) {
    ...
    if (!this.friends) {
      this.friends = [userEmail];
      status = true;
    } else if (this.friends.indexOf(userEmail) === -1) {
        // add friend
      this.friends.push(userEmail);
      status = true;
    } else {
      status = false;
    }
  ...
  removeFriend: function (userEmail) {
    let status;
    ...
    if (!this.friends) {
      ...
      // remove friend
      const index = this.friends.indexOf(userEmail);
      if (index > -1) {
